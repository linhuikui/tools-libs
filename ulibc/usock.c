...
static int usock_connect(int type, struct sockaddr *sa, int sa_len, int family, int socktype, bool server, int maxconn)
{
	int sock;
	...
	maxconn = maxconn < SOMAXCONN ? maxconn : SOMAXCONN;
	
	if (!bind(sock, sa, sa_len) &&
		    (socktype != SOCK_STREAM || !listen(sock, maxconn)))
			return sock;
	} else {
...
static int usock_unix(int type, const char *host, int socktype, bool server, int maxconn)
{
	...
	return usock_connect(type, (struct sockaddr*)&sun, sizeof(sun), AF_UNIX, socktype, server, maxconn);
}

static int usock_inet(int type, const char *host, const char *service, int socktype, bool server, int maxconn)
{
	...
	for (rp = result; rp != NULL; rp = rp->ai_next) {
		sock = usock_connect(type, rp->ai_addr, rp->ai_addrlen, rp->ai_family, socktype, server, maxconn);
		if (sock >= 0)
			break;
	}
...
int usock(int type, const char *host, const char *service, int maxconn) {
	int socktype = ((type & 0xff) == USOCK_TCP) ? SOCK_STREAM : SOCK_DGRAM;
	bool server = !!(type & USOCK_SERVER);
	int sock;

	if (type & USOCK_UNIX)
		sock = usock_unix(type, host, socktype, server, maxconn);
	else
		sock = usock_inet(type, host, service, socktype, server, maxconn);
